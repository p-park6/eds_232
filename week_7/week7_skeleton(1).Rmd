...
```{r}
redlining = read_csv(here::here("..", "eds_232", "week_7", "data", "redlining.csv")) %>% 
...
rf_model <- rand_forest(mtry = tune(), trees = tune()) %>% 
  set_engine("ranger", verbose = TRUE, importance = "impurity") %>%  #adding extra variables: verbose is part of the ranger package
#we are changing the importance parameter since it defaults to none
  set_mode("regression") #mode is set to regression since that is what the data contains
```
...
#because slope is lower than one, it underpredicts
#test data is opposite of training data (it is a negative slope)
#less than one slope(underpredicting)
...
#personal grid that we created
my_grid = expand.grid(mtry = seq(2, 6, length.out = 5), trees = round(seq(50, 2000, length.out = 10))) #make sure these match the parameter we want to tune

rf_cv <- rf_wflw %>%
   tune_grid(resamples = folds, grid = my_grid) #for grid you put in the grid you created
```
...
```{r}
rf_final2 <- finalize_workflow(rf_wflw, select_best(rf_cv, metric = "rmse"))
```
