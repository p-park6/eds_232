...
#gradient boosting tries to find the maximum (finding the 'slope' on the graph (quadratic graph))
#high learn rate may skip the maximum, you need a low enough learn rate to look for the maximum
...
fish_clean = read_csv(here("week_6", "data", "fish_clean(1).csv"))
...
```{r, include=FALSE}
load(file = here::here("week_6", "data",  "rf_tune.rda"))
```
...
# randomly selected predictor is more important in this specific pattern
# always different in different models
```
...
#### Model 5: Boosted Trees Classifier
...
```{r, include=FALSE}
load(file = here::here("week_6","data",  "xgb_tune.rda"))
```
...
```{r, eval = FALSE}
#best model will be highest roc that we choose to base it on
...
```{r}
#just for that class (only one habitat)
...
#predictions for all of the class (all of the habitats)
...
#can say that they both fit the data well
# can redo to see if we can fit it in a specific range
```
