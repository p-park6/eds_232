...
#what is this workflow doing? how is it different than the one below?
...
```{r}
#Fit workflow
#this might be an example of what not to do
...
#what is the difference between this one and the other workflow? is this one being set up to tune the grid?
...
```{r}
#no tuning for rbf but tuning needed  for linear
...
```{r}
#Plot the fit (we can't do because our values are numeric)
svm_rbf_fit %>%
  extract_fit_engine() %>%
  plot()
```
