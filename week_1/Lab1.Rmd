...
- **Answer**: While I wouldn't say that this plot is useful, I would say that one of the information that I can gain from it is how consistent the price stays as the days go by.
...
- **Answer**: There is one city that is represented in the dataset.
...
-**Answer**: Out of the three variables that I have tested, the variable package is highly correlated with price.
...
- **Answer**: While the outputs that we get are continuous, meaning that they only contain whole numbers, the inputs that we are putting in for packages are discrete. This means that the model does not know what the price is for a half of a bundle and has to estimate with the data it is given from the price of 0 bundles and 1 bundle.
